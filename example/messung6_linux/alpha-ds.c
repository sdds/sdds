...
#define SDDS_TOPIC 0x10


rc_t TopicMarshalling_Alpha_cpy(Data dest, Data source);

#ifdef SDDS_TOPIC_HAS_PUB

rc_t TopicMarshalling_Alpha_decode(byte_t* buffer, Data data, size_t* size);
...
/*
DDS_ReturnCode_t DDS_AlphaDataReader_set_listener(
	DDS_DataReader _this,
...
rc_t TopicMarshalling_Alpha_encode(byte_t* buffer, Data data, size_t* size);
...
#ifdef SDDS_TOPIC_HAS_SUB
	topic->Data_encode = TopicMarshalling_Alpha_encode;
	//topic->dsinks.list = locator;
#endif

#ifdef SDDS_TOPIC_HAS_PUB
	topic->Data_decode = TopicMarshalling_Alpha_decode;
#endif

	topic->domain = SDDS_DOMAIN;
	topic->id = SDDS_TOPIC;
	topic->Data_cpy = TopicMarshalling_Alpha_cpy;
...
rc_t TopicMarshalling_Alpha_cpy(Data dest, Data source)
{
	memcpy(dest, source, sizeof(Alpha));
