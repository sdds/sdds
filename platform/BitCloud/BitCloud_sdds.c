...
rc_t
BitCloud_sdds_processFrame(APS_DataInd_t* indData) {
    // get locator
...
rc_t
Network_createLocator(Locator* loc) {
...
bool_t
Locator_isEqual(Locator l1, Locator l2) {
    ...
    switch (a->addrMode) {
    case (APS_GROUP_ADDRESS):
        if (a->addr.groupAddress == b->addr.groupAddress) {
            return true;
        }
        break;
    case (APS_SHORT_ADDRESS):
        if (a->addr.shortAddress == b->addr.shortAddress) {
            return true;
        }
        break;
    case (APS_EXT_ADDRESS):
        if (a->addr.extAddress == b->addr.extAddress) {
            return true;
        }
        break;
    default:
